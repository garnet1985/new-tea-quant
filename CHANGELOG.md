...
### v0.3.2 (2026-5-20)
- 为非开发用户去除了Nodejs依赖 - Done
- 增加market profile模块，细化回测时候的交易规则 - Done
- 在userspace中提供了可以复写/添加新的market profile的配置 - Done
- 最小买入手数从100股变成了不同板块不同股数 - Done
- 加入涨停和跌停时限制交易的配置 - Done
- 根据不同股票或不同上市阶段，限制不同最大最小涨跌幅度 - Done
- 重新划分了project context模块的职责边界 - Done
- 新增加了一些UI样式：比如读取的动画，随机背景 - Done
- 优化UX，为很多地方添加了解释的tooltip。对次要内容使用tooltip进行归纳。最后增强了tooltip的视觉效果。- IPG
- 修复了核心设置的json发生变化就会导致整个设置区域消失的bug - Done
- 为核心参数设置增加了大窗口编辑模式 - Done
- 优化了策略工作台执行步骤的进度显示 - NS
- 修复了工作台版本差异状态的变化显示错误 - Done
- UI header上的版本号改为从API获取 - Done
