...
- 给决策者模式的每次投资加入笔记的功能，在回溯日历里能看到当时的笔记
- 改进了launcher.py每次启动不会再卡在更新pip上
- 修复了install.py会退出的bug
- [ ] K线与指标的显示需要测试
- [ ] 决策者模式的st区间似乎有问题
- [ ] 决策者模式应该在用户做决策的时候可以有记笔记的功能，能记住用户为什么做这个决策
- [ ] 我们的price factor逐股图上最好带上止损和止盈的线（hover到点上的时候？否则会很乱）
- [ ] 加入平均资金利用率和最高利用率的report和track
- [ ] 将backtest engine放入infra内
- [ ] 将indicator 放入infra内
- [ ] 将export和import 放入modules模块内
- [ ] 让安装时候的数据导入变成多进程，从而加速安装
- [ ] 对所有modules加上了package声明
...
- 全新模拟回测的第四步：决策模拟
- 集成AI模型和必要文档（需要自己加入API key进行运行）
- 对策略内页加入帮助引导
- 资金模拟（第三层内）加入Sharpe和Sortino比率
- Bugfix：修复了一些小的bug
...
- 回测归因：加强分析模块，可以通过统计和机器学习手法分析出策略实现的归因问题
- 分离了机器学习的安装流程，重新更新了安装体验，增加了新的UI的欢迎页面
- 加强策略中的is_stop_loss和is_take_profit函数，完全支持自定义目标，并加入新的布林轨策略进行演示
- 把第三层回测的收益和回撤图进行了合并
- 对所有演示策略加上了归因演示代码
- 重新整理和定义了策略运行的版本和指纹系统，现在系统不再依赖db的缓存，UI和后端可以同时修改settings.py并不会导致版本错乱
- 策略演示页面加上了回到策略选择页面的快速入口
- 为策略版本增加了新的状态：即将清理（缓存过期）和仅供查阅（环境指纹改变），增加了pin和删除版本的功能，被pin过的策略版本不会被自动清理，删除版本会清理版本的所有缓存的结果
- 前端UI升级react版本，从17.x升级到18.x
- 对前端所有请求进行了错误和超时处理，以免任何错误卡住UI的操作
- 统一处理了UI上很多会导致用户操作卡死的异常处理
- 统一处理了后端程序里很多对错误的跳过（pass）操作
