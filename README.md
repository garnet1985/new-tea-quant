...
- **NTQ 方案**：内置贴近真实市场的回测引擎。框架在底层默认处理了几类最容易让回测失真的问题：
  - **幸存者偏差**：使用 PIT（point-in-time）股票池来防范
  - **交易规则限制**：自动遵守买入手数规则、涨跌停无法成交、T+1 等
  - **未来函数**：数据按日期严格切割，防止「上帝视角」式回测  
...
- **策略信息**：顶部全宽区域，展示名称、说明、版本等，以及发布等全局操作。
- **策略配置**：左侧面板，随回测步骤变化。改参数会生成新版本，便于对比调试。  
...
**注意：** 调试好的策略必须先在策略内页标题下方点击 **「发布策略」**，才会真正用于扫描。否则参数只存在于工作台缓存，不会写回策略代码，扫描也不会用到你刚调好的版本。
