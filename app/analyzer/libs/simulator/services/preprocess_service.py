...
class PreprocessService:
    ...
    def validate_simulate_one_day_func(func: Optional[Callable]) -> bool:
        ...
        """
        if func is None:
            logger.warning("⚠️ 未提供单日模拟函数，将使用默认实现")
            return True
        
        logger.info("🔍 验证单日模拟函数签名...")
        ...
        try:
            ...
            logger.info(f"✅ 单日模拟函数验证通过: {func.__name__}")
            return True
            
        except Exception as e:
            logger.error(f"❌ 单日模拟函数验证失败: {e}")
            raise ValueError(f"单日模拟函数验证失败: {e}")
    
    @staticmethod
    def preprocess(settings: Dict[str, Any], simulate_one_day_func: Optional[Callable] = None) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
        """
        完整的预处理流程
        
        Args:
            settings: 原始设置
            simulate_one_day_func: 单日模拟函数（可选）
            
        Returns:
            tuple: (validated_settings, stock_list)
        """
        # 验证设置
        validated_settings = PreprocessService.validate_settings(settings)
        
        # 验证函数（如果提供）
        if simulate_one_day_func is not None:
            PreprocessService.validate_simulate_one_day_func(simulate_one_day_func)
        
        # 获取股票列表
        stock_list = PreprocessService.get_stock_list(validated_settings)
