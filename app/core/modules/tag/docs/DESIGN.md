...
**位置**：`app/userspace/tags/<scenario_name>/settings.py`

**职责**：定义业务场景和标签的配置信息

**配置结构**：

> **注意**：详细的配置结构请参考 `app/userspace/tags/example_settings.py`，该文件包含完整的配置示例和每个属性的详细解释。
...
- `incremental_required_records_before_as_of_date`: INCREMENTAL 模式下需要的历史记录数（必须）
- `core`: 业务核心参数（可选）
- `performance`: 性能配置（可选）
  - `data_chunk_size`: 数据切片大小（默认 500，最小 300）
  - `use_chunk`: 是否使用 chunk 模式（默认 True）
- `tags`: Tag 级别配置列表（必须）
...
**位置**：`app/userspace/tags/<scenario_name>/tag_worker.py`
...
class MomentumTagWorker(BaseTagWorker):
    ...
    def calculate_tag(
        ...
        tag_description = tag_definition.description
...
**位置**：`app/core/modules/tag/core/base_tag_worker.py`
...
1. **执行流程**
   - `process_entity()`: 子进程 worker 方法（处理单个 entity）
     - `_preprocess()`: 预处理（获取交易日列表，INCREMENTAL 模式下初始化数据加载）
...
**位置**：`app/core/modules/tag/core/components/tag_worker_helper/tag_worker_data_manager.py`
...
1. **数据需求解析**
   ...
   - 提取 `data_slice_size`（默认 500，最小 300）
   - 提取 `use_chunk`（默认 True）
   - 提取 `incremental_required_records_before_as_of_date`（INCREMENTAL 模式必须）

2. **数据加载策略**

   **Kline 数据加载**：
   - **Chunk 模式**（`use_chunk=True`）：
     - 滑动窗口策略：保持最多 2 个 chunk 的数据在内存中
     - 按记录数切片加载（默认 500 条/切片）
     - 当需要加载第三个 chunk 时，自动删除第一个 chunk
     - INCREMENTAL 模式初始化：自动加载包含 `start_date` 的当前 chunk 和前一个 chunk
   ...
   **Required Data 加载**：
   - 全量加载到 `as_of_date`（不做 chunk，因为数据量小）
   - 在 INCREMENTAL 模式下，根据 base data 的第一个记录时间加载 lookback data

   **非 Kline Base Entity 加载**：
   - 全量加载策略（类似 required_data）
   - 支持季度数据（`corporate_finance`）和日期数据
...
5. **INCREMENTAL 模式初始化**
   - `initialize_for_incremental(start_date)`: 在 INCREMENTAL 模式下初始化数据加载
   - 如果 `use_chunk=True`：加载当前 chunk 和前一个 chunk（共 2 个 chunk）
   - 如果 `use_chunk=False`：全量加载到 `start_date`

**配置验证**：
- `data_chunk_size` 必须 >= 300
- 如果 `use_chunk=True` 且 `incremental_required_records_before_as_of_date > data_chunk_size`，抛出错误
...
**责任边界**：
- ✅ 从 settings 解析数据需求
- ✅ 管理数据缓存（chunk 模式或全量模式）
- ✅ 提供数据加载和过滤接口
- ✅ 保证数据加载的一致性（根据 settings 配置）
- ✅ 支持 INCREMENTAL 模式的初始化
- ❌ 不负责业务计算逻辑

### 5. TagManager (`tag_manager.py`)

**位置**：`app/core/modules/tag/core/tag_manager.py`
...
1. **发现和注册**
   - `_discover_scenarios_from_folder()`: 发现所有业务场景
   - 自动加载 settings 和 worker 类

2. **管理接口**
   - `execute(scenario_name=None, settings=None)`: 执行所有或单个 scenario
   - `get_scenario(scenario_name)`: 获取指定 scenario 的信息
   - `list_scenarios()`: 列出所有 scenario 名称
   - `refresh_scenario()`: 重新发现所有 scenarios

3. **多进程调度**
   - `_execute_single()`: 执行单个 scenario 的多进程计算
...
> **详细配置结构请参考** `app/userspace/tags/example_settings.py`，该文件包含完整的配置示例和每个属性的详细解释。
...
- **业务配置**：`core`, `performance`（可选）
  - `performance.data_chunk_size`：数据切片大小（默认 500，最小 300）
  - `performance.use_chunk`：是否使用 chunk 模式（默认 True）
- **Tag 配置**：`tags`（必须，至少一个）
...
4. 循环执行每个 scenario（scenario 之间同步操作）：
   ...
   d. 构建 jobs 并执行多进程计算
5. 计算流程（子进程中）：
   a. INCREMENTAL 模式下初始化数据加载
   b. 遍历每个交易日
   c. 对每个日期，获取历史数据并过滤到 as_of_date
   d. 对每个 tag 调用 calculate_tag()
   e. 批量保存结果
```
...
tag_manager.execute()  # 或 tag_manager.execute(scenario_name="market_value")
```

**TagManager.execute() 流程**：

1. **发现和注册 Scenarios**
   - `_discover_scenarios_from_folder()`: 遍历 scenarios 目录，加载 settings 和 worker 类
...
2. 预处理：获取交易日列表，INCREMENTAL 模式下初始化数据加载
...
**Required Data**：
- 全量加载到 as_of_date（不做chunk，因为数据量小）
- 在 INCREMENTAL 模式下，根据 base data 的第一个记录时间加载 lookback data

**内存控制**：
- 每个进程只处理一个 entity
- Chunk 模式：最多保持 2 个 chunk（约 1000 条记录）
- 全量模式：一次性加载所有数据
- 进程结束自动释放内存
...
#### 6. 为什么支持 Chunk 和全量两种模式？

**原因**：
- Chunk 模式：适合大数据量场景，内存可控
- 全量模式：适合小数据量场景，逻辑简单
- 用户可以根据场景选择
...
### 5. 为什么在 TagManager 中检查 is_enabled？
...
### 8. 为什么 INCREMENTAL 模式需要初始化？

**原因**：
- 需要确保有足够的历史数据用于计算
- 自动加载当前 chunk 和前一个 chunk，保证数据完整性
- 简化用户配置，减少出错可能
...
**业务场景 vs Tag**：
- **业务场景**（文件夹名）：如 `momentum`（动量因子）
- **Tag**（settings 中定义）：如 `momentum_60_days`, `momentum_120_days`
- 一个业务场景可以产生多个 tags
...
**TagUpdateMode**（`app/core/modules/tag/core/enums.py`）：
- `INCREMENTAL`: 增量更新
- `REFRESH`: 全量刷新

详细枚举定义请参考 `app/core/modules/tag/core/enums.py`。
