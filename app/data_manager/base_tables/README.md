...
基础表是系统运行所必需的核心数据表，与业务逻辑紧密相关，由 `DataLoader` 统一管理。
...
- ✅ Schema 定义在 `app/data_loader/base_tables/`
- ✅ 由 `DataLoader` 统一管理和创建
- ✅ 在系统初始化时自动创建
...
```bash
# 创建新表目录
mkdir -p app/data_loader/base_tables/my_table

# 创建 schema.json
cat > app/data_loader/base_tables/my_table/schema.json << 'EOF'
{
    "name": "my_table",
    "primaryKey": "id",
    "fields": [
...
基础表的数据访问通过 `DataLoader` 及其子 Loader：
