...
class CPIHandler(BaseDataSourceHandler):
    """
    CPI 数据 Handler
    ...
    async def fetch(self, context: Dict[str, Any] = None) -> List:
        ...
        if not start_date or not end_date:
            raise ValueError("CPI Handler 需要 start_date 和 end_date 参数")
        
        logger.debug(f"获取 CPI 数据: {start_date} 至 {end_date}")
    ...
    async def normalize(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        从 Tushare 返回的 DataFrame 中提取 CPI 数据，进行字段映射
        """
        # 使用辅助方法获取简单 Task 的结果
        df = self.get_simple_result(raw_data)
        
        if df is None or df.empty:
            logger.warning("CPI 数据查询返回空数据")
            return {"data": []}
        ...
        for item in records:
            ...
            # API 字段：month, nt_val, nt_yoy, nt_mom
            ...
            mapped = {
                "date": month,
                "cpi": float(item.get('nt_val', 0)) if item.get('nt_val') is not None else 0.0,
                "cpi_yoy": float(item.get('nt_yoy', 0)) if item.get('nt_yoy') is not None else 0.0,
                "cpi_mom": float(item.get('nt_mom', 0)) if item.get('nt_mom') is not None else 0.0,
            }
            
            formatted.append(mapped)
        
        logger.info(f"✅ CPI 数据处理完成，共 {len(formatted)} 条记录")
