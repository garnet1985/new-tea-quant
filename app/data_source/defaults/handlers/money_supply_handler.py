...
class MoneySupplyHandler(BaseDataSourceHandler):
    """
    货币供应量数据 Handler
    ...
    # 类属性（必须定义）
    data_source = "money_supply"
    renew_type = "incremental"  # 增量更新
    description = "获取货币供应量数据（月度）"
    dependencies = []  # 无依赖
    ...
    async def fetch(self, context: Dict[str, Any] = None) -> List:
        ...
        2. 调用 Tushare money_supply API 获取月度数据
        """
        context = context or {}
        ...
        if not start_date or not end_date:
            raise ValueError("Money Supply Handler 需要 start_date 和 end_date 参数")
        
        logger.debug(f"获取货币供应量数据: {start_date} 至 {end_date}")
    ...
    async def normalize(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        从 Tushare 返回的 DataFrame 中提取货币供应量数据，进行字段映射
        """
        # 使用辅助方法获取简单 Task 的结果
        df = self.get_simple_result(raw_data)
        
        if df is None or df.empty:
            logger.warning("货币供应量数据查询返回空数据")
            return {"data": []}
        ...
        for item in records:
            ...
            # API 字段：month, m0, m0_yoy, m0_mom, m1, m1_yoy, m1_mom, m2, m2_yoy, m2_mom
            ...
            mapped = {
                "date": month,
                "m0": float(item.get('m0', 0)) if item.get('m0') is not None else 0.0,
                "m0_yoy": float(item.get('m0_yoy', 0)) if item.get('m0_yoy') is not None else 0.0,
                "m0_mom": float(item.get('m0_mom', 0)) if item.get('m0_mom') is not None else 0.0,
                "m1": float(item.get('m1', 0)) if item.get('m1') is not None else 0.0,
                "m1_yoy": float(item.get('m1_yoy', 0)) if item.get('m1_yoy') is not None else 0.0,
                "m1_mom": float(item.get('m1_mom', 0)) if item.get('m1_mom') is not None else 0.0,
                "m2": float(item.get('m2', 0)) if item.get('m2') is not None else 0.0,
                "m2_yoy": float(item.get('m2_yoy', 0)) if item.get('m2_yoy') is not None else 0.0,
                "m2_mom": float(item.get('m2_mom', 0)) if item.get('m2_mom') is not None else 0.0,
            }
            
            formatted.append(mapped)
        
        logger.info(f"✅ 货币供应量数据处理完成，共 {len(formatted)} 条记录")
