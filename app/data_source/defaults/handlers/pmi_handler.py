...
class PMIHandler(BaseDataSourceHandler):
    """
    PMI 数据 Handler
    ...
    description = "获取 PMI 采购经理人指数数据（月度）"
    dependencies = []  # 无依赖
    ...
    async def fetch(self, context: Dict[str, Any] = None) -> List:
        ...
        if not start_date or not end_date:
            raise ValueError("PMI Handler 需要 start_date 和 end_date 参数")
        
        logger.debug(f"获取 PMI 数据: {start_date} 至 {end_date}")
    ...
    async def normalize(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        从 Tushare 返回的 DataFrame 中提取 PMI 数据，进行字段映射
        """
        # 使用辅助方法获取简单 Task 的结果
        df = self.get_simple_result(raw_data)
        
        if df is None or df.empty:
            logger.warning("PMI 数据查询返回空数据")
            return {"data": []}
        ...
        for item in records:
            ...
            # API 字段：MONTH, PMI010000, PMI010100, PMI010200, PMI010300
            ...
            mapped = {
                "date": month,
                "pmi": float(item.get('PMI010000', 0)) if item.get('PMI010000') is not None else 0.0,
                "pmi_l_scale": float(item.get('PMI010100', 0)) if item.get('PMI010100') is not None else 0.0,
                "pmi_m_scale": float(item.get('PMI010200', 0)) if item.get('PMI010200') is not None else 0.0,
                "pmi_s_scale": float(item.get('PMI010300', 0)) if item.get('PMI010300') is not None else 0.0,
            }
            
            formatted.append(mapped)
        
        logger.info(f"✅ PMI 数据处理完成，共 {len(formatted)} 条记录")
