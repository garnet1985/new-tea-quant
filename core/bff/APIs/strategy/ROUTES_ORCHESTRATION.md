...
core/bff/APIs/strategy/
  ...
  stack.py                  # 懒加载 launcher（仅 runner / scan）
  routes/
    catalog/                # V2-02
    package/                # V2-13 … 15
    report/                 # V2-07*
    ...
    runner/                 # V2-05/06* + scan（迁移中）
  helpers/
```

## 原则

- **路径**：有 target 时统一为 `/v1/strategy/{strategy_key_or_name}/…`（``meta.key`` 或 path name，可多段）；无 target 的全局资源（catalog、settings 选项、package import、全表 cache）不加 strategy 段。
...
| scan | * | `/v1/strategy/<name>/scan*` | `routes/runner/` |
