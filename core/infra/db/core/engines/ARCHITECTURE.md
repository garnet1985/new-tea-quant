...
4. **重复优于过早抽象** — MySQL 与 PostgreSQL 相似处允许复制；仅在重复造成维护痛点时，才抽到可选的 `shared/`  helpers。
...
| `engines/shared/fields` | 跨 backend 的 **schema 语言**；parser 负责 field → DDL |
...
仅当**同时满足**以下条件时，才将代码放入 `engines/shared/`：
...
DuckDB 三存储域（data / tag / strategy）的**产品决策**见 [决策 6](../docs/DESIGN.md) 与 [storage-domains.md](../docs/storage-domains.md)。
...
- [../docs/DESIGN.md](../docs/DESIGN.md) — 决策 7–11
