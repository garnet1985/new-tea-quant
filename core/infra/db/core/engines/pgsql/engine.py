...
class _WriteQueueHost:
    def __init__(self, engine: PgsqlEngine) -> None:
        self._engine = engine
...
class PgsqlEngine(DbEngineAbc):
    ...
    def __init__(self, meta: EngineConfigMeta, *, is_verbose: bool = False) -> None:
        ...
        self.connector = PgsqlConnector(meta.require_pgsql(), is_verbose=is_verbose)
    ...
    def initialize(self) -> None:
        ...
        if self.is_verbose:
            logger.debug("postgresql Engine 初始化完成")
    ...
    def table_operator(self, table_name: str) -> DbTableAbc:
        ...
        if cached is not None:
            return cached
        op = PgsqlTableOperator(self, name)
