"""MySQL / PostgreSQL 共用的 server engine 编排基类。"""
...
from core.infra.db.core.engines.shared import dialect
from core.infra.db.core.schema_manager import SchemaManager
from core.infra.db.core.table_queriers.services.batch_operation_queue import BatchWriteQueue

logger = logging.getLogger(__name__)


class WriteQueueHost:
    """BatchWriteQueue 挂载的 engine 宿主。"""

    def __init__(self, engine: "ServerEngineBase") -> None:
        self._engine = engine
...
class ServerEngineBase(DbEngineAbc):
    """MySQL / PgSQL 共享编排；子类只提供 connector 与 table_operator 工厂。"""

    engine_key: str = ""

    def __init__(self, meta: EngineConfigMeta, *, is_verbose: bool = False) -> None:
        super().__init__(meta, is_verbose=is_verbose)
        self.connector = self._create_connector(meta, is_verbose=is_verbose)
    ...
    def initialize(self) -> None:
        ...
        if self.is_verbose:
            logger.debug("%s Engine 初始化完成", self.engine_key)
    ...
    def table_operator(self, table_name: str) -> DbTableAbc:
        ...
        if cached is not None:
            return cached
        op = self._create_table_operator(name)
    ...
    def create_all_tables(
        self, schemas: Optional[Dict[str, Dict[str, Any]]] = None
    ) -> None:
        self._require_ready()
        if schemas is None:
    ...
    def _init_write_queue(self) -> None:
        ...
        self._write_queue = BatchWriteQueue(
            table_manager=WriteQueueHost(self),
