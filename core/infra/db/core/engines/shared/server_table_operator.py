"""MySQL / PostgreSQL 共用的表级 CRUD 基类。"""
...
from core.infra.db.core.engines.shared import dialect, row_sql
from core.infra.db.core.table_queriers.services.batch_operation import BatchOperation

logger = logging.getLogger(__name__)


class ServerTableOperatorBase(DbTableAbc):
    """子类设置 ``database_type`` / ``supports_delete_limit``。"""

    database_type: str = ""
    supports_delete_limit: bool = False
    ...
    def _sql_table(self) -> str:
        return dialect.sql_qualify_table_name(
            self._engine.meta.raw_config, self.table_name
        )
    ...
    def delete(
        ...
        if limit is not None and self.supports_delete_limit:
            query += f" LIMIT {limit}"
        max_retries = 3
        retry_delay = 0.1
