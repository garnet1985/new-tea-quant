...
from core.infra.db.core.migration.introspection import introspect_database
...
from core.infra.db.core.migration.schema_diff import SchemaDiffResult, diff_expected_schemas
...
class MigrationRunResult:
    """单次 ``MigrationRunner.run`` 的结果（plan 驻内存）。"""
...
class MigrationRunner:
    """schema 升级编排实现类（公开入口 Db.migration → MigrationManager）。"""

    PRE_MIRROR_FILENAME = DEFAULT_PRE_MIRROR_FILENAME
    ...
    def default_pre_mirror_snapshot_path(repo_root: Path) -> Path:
        ...
        return (
            ...
            / "update"
            / "cache"
            / MigrationRunner.PRE_MIRROR_FILENAME
        ).resolve()
    ...
    def load_current_table_schemas(
        ...
        *,
        ...
        if tables_dir is not None:
            ...
            td = str(ProjectContext.path.get_core_root() / "tables")
    ...
    def run(
        *,
        ...
        Args:
            ...
            against_database: ``True`` 时连库 introspection 并裁剪 plan
            database_type: 方言；连库时以 ``DatabaseManager`` 配置为准
        """
        result = MigrationRunResult()
        old_schemas = MigrationRunner.load_schemas_from_snapshot(pre_mirror_snapshot)
        new_schemas = MigrationRunner.load_current_table_schemas(
        ...
        if use_db:
            ...
            try:
                ...
                diff, plan = MigrationRunner.build_migration_plan(
                    old_schemas,
                    new_schemas,
        ...
        diff, plan = MigrationRunner.build_migration_plan(
            old_schemas, new_schemas, database_type=db_type
        )
        result.diff = diff
        result.plan = plan
        result.step_count = len(ordered_plan(plan))
