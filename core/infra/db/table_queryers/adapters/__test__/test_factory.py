...
class TestDatabaseAdapterFactory:
    ...
    def test_create_from_legacy_config_with_db_path(self):
        """测试从旧配置创建（db_path）"""
        config = {
            'db_path': ':memory:'
        }
        ...
        with patch('core.infra.db.adapters.factory.SQLiteAdapter') as mock_adapter_class:
            ...
            adapter = DatabaseAdapterFactory.create_from_legacy_config(config)
            mock_adapter_class.assert_called_once()
    
    def test_create_from_legacy_config_with_host_mysql(self):
        """测试从旧配置创建（host + port 3306 = MySQL）"""
        config = {
            'host': 'localhost',
            'database': 'test_db',
            'port': 3306
    ...
    def test_create_from_legacy_config_with_host_postgresql(self):
        """测试从旧配置创建（host + port 5432 = PostgreSQL）"""
        config = {
            'host': 'localhost',
            'database': 'test_db',
            'port': 5432
