"""MachineInfo / MachineCapacity 单测。"""
...
def test_get_cpu_count_positive() -> None:
    assert MachineInfo.get_cpu_count() >= 1
    assert MachineInfo.get_cpu_count() == (mp.cpu_count() or 1)
...
def test_parse_max_parallel_jobs_cap() -> None:
