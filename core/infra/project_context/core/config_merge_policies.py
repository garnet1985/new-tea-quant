...
公开入口：``ProjectContext.config.merge_market_profile_dicts``。
"""
        ...
        for key, user_val in user.items():
            if key == "rules":
                ...
                if not isinstance(core_rules, dict):
                    merged["rules"] = copy.deepcopy(user_val)
                    continue
                merged["rules"] = ConfigMergePolicies._merge_rules_map(core_rules, user_val)
                continue

            if isinstance(user_val, dict) and isinstance(merged.get(key), dict):
                merged[key] = ConfigMergePolicies._merge_nested_dict(merged[key], user_val)
            else:
    ...
    def _merge_rules_map(
        ...
        for rule_type, user_block in user_rules.items():
            ...
            if isinstance(core_block, dict):
                out[rule_type] = ConfigMergePolicies._merge_rule_block(core_block, user_block)
            else:
                out[rule_type] = copy.deepcopy(user_block)
        return out
    ...
    def _merge_rule_block(
        ...
        for key, val in user_block.items():
            ...
            if isinstance(val, dict) and isinstance(merged.get(key), dict):
                merged[key] = ConfigMergePolicies._merge_nested_dict(merged[key], val)
            else:
        ...
        if user_list is not None:
            ...
            if isinstance(core_list, list):
                merged["rules"] = ConfigMergePolicies._merge_rule_entries_by_key(
                    core_list, user_list
                )
            else:
    ...
    def _merge_rule_entries_by_key(
        ...
        for item in user_entries:
            ...
            if entry_key in by_key:
                by_key[entry_key] = ConfigMergePolicies._merge_nested_dict(
                    by_key[entry_key], item
                )
            else:
                by_key[entry_key] = copy.deepcopy(item)
                order.append(entry_key)
    ...
    def _merge_nested_dict(base: Dict[str, Any], overlay: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, val in overlay.items():
            if isinstance(val, dict) and isinstance(out.get(key), dict):
                out[key] = ConfigMergePolicies._merge_nested_dict(out[key], val)
            else:
                out[key] = copy.deepcopy(val)
        return out


__all__ = ["ConfigMergePolicies"]
