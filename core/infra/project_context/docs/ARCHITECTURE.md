...
**核心设计：** Facade 模式。包根只导出 `ProjectContext`；类型与常量走 `contracts`；`PathManager` / `ConfigManager` / `DiscoveryManager` / `FileManager` 为内部实现。

---

## 模块目标

- 单一入口：`ProjectContext` + namespace（`path` / `config` / `meta` / `cache` / `discovery`）
- 稳定路径构造，避免业务硬编码相对路径
...
对外：
  ProjectContext
    ├── path / config / meta / cache / discovery
    └── contracts（异常、常量、合并函数）

内部（core/）：
  PathManager · ConfigManager · DiscoveryManager · FileManager
```
