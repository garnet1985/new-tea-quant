...
`infra.setup` 是安装域：判断是否需要安装、跑步骤流水线、生产 `init_userspace` / `init_data` 产物、UI/CLI 安装编排。
...
**Out of scope：** 应用升级（见 `infra.updater`）；zip 落盘目录（仓库根 `init_userspace/`、`init_data/`）；用户策略/打标命令
...
init_userspace/ init_data/   # 仓库根产物
```
