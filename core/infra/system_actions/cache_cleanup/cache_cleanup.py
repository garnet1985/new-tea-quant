...
def clear_backtest_results_disk(*, strategy_names: Optional[Iterable[str]] = None) -> int:
    ...
    for name in _discovered_strategy_keys(strategy_names=strategy_names):
        sim_root = PathManager.get_strategy_directory_results(name) / "simulations"
        if sim_root.exists():
            _rm_tree(sim_root)
            removed += 1
    return removed
...
def clear_scan_results_disk(*, strategy_names: Optional[Iterable[str]] = None) -> int:
    ...
    removed = 0
    for name in _discovered_strategy_keys(strategy_names=strategy_names):
        scan_root = PathManager.get_strategy_directory_scan_results(name)
        if scan_root.exists():
            _rm_tree(scan_root)
            removed += 1
    return removed
...
def clear_strategy_results_disk(*, strategy_names: Optional[Iterable[str]] = None) -> int:
    ...
    removed = 0
    for name in _discovered_strategy_keys(strategy_names=strategy_names):
        results = PathManager.get_strategy_directory_results(name)
        if results.exists():
            _rm_tree(results)
            removed += 1
    return removed
...
def clear_userspace_ntq_dir() -> None:
    ...
    us_ntq = PathManager.get_userspace_ntq_directory()
    if us_ntq.is_dir():
        _rm_tree(us_ntq)
