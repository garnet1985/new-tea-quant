...
from .flush_service import TraceFlushService

logger = logging.getLogger(__name__)


class TraceDrainService:
    """Daemon thread that periodically flushes the local queue."""
                ...
                while not cls._stop.wait(timeout=cls._interval_sec()):
                    try:
                        if not TraceConfigService.is_enabled():
                            continue
                        TraceFlushService.flush(budget=FlushBudget.AUTO)
    ...
    def _atexit_flush(cls) -> None:
        try:
            cls._stop.set()
            TraceFlushService.flush(budget=FlushBudget.STANDARD)
        except Exception:
            pass
