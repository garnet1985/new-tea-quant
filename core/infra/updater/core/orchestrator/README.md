...
| 执行者 | 始终 **`userspace/system/updater`**；zip 内 **不执行任意未校验脚本**（迁移入口由 updater **spawn 新解释器**调用新 `core` 中的模块）。 |
...
| **`userspace/system/updater/`** | 长期 bootstrap：停服、镜像、收尾编排 |
...
| 11 | `_trigger_core_extra_actions` | 子进程 ``python -m core.infra.updater.core.post_upgrade run``；``core/infra/updater/core/post_upgrade`` 注册表 **为空则跳过**；日志 ``logs/post-upgrade-<UTC>.log`` → ``ctx.post_upgrade``；``NTQ_UPDATE_SKIP_POST_UPGRADE=1`` 可跳过整步 |
...
### 为何必须在 ``userspace/system/updater/``，而不是 ``core/`` 或 ``setup/``？

应用升级会替换 ``managed_scope`` 下的 ``core/``、``setup/``；若在**正在执行的那份代码路径**里跑镜像，会遇到 **文件占用 / 无法覆盖**（尤其 Windows）。因此：

- **写盘升级**的编排脚本放在 **`userspace/system/updater/`**，由 ``Updater.runtime.sync_orchestrator`` 写入。
...
| 路径 | 说明 |
|------|------|
| **`core/infra/updater/core/orchestrator/pipeline.py`** | 与运行时 **`userspace/system/updater/pipeline.py`** 同源。 |
| **`core/infra/updater/core/orchestrator/run_apply.py`** | 开发调试：``python core/infra/updater/core/orchestrator/run_apply.py``。 |
...
| 2026-08-13 | 编排源码迁入 ``core/infra/updater/core/orchestrator/``；运行时仍为 ``userspace/system/updater/`` |
