...
def _preflight_post_upgrade_module(repo_root: Path, py: Path, env: Dict[str, str]) -> None:
    r = subprocess.run(
        [str(py), "-c", "import core.infra.updater.core.post_upgrade"],
        cwd=str(repo_root),
        env=env,
        capture_output=True,
        text=True,
    ...
    if r.returncode != 0:
        detail = (r.stderr or r.stdout or "").strip()
        raise RuntimeError(
            "NTQ updater: cannot import core.infra.updater.core.post_upgrade "
            f"(check PYTHONPATH/venv). {detail}"
        )
...
def spawn_post_upgrade_actions_cli(repo_root: Path) -> PostUpgradeResult:
    """
    子进程调用 ``python -m core.infra.updater.core.post_upgrade run``。

    执行 ``core/infra/updater/core/post_upgrade`` 中已注册的收尾动作；注册表为空时子进程正常退出（跳过）。
    ...
    cmd: List[str] = [
        str(py),
        "-m",
        "core.infra.updater.core.post_upgrade",
        "run",
        "--repo-root",
        str(repo_root),
        "--result-json",
...
def reinstall_runtime_dependencies_cli(repo_root: Path, *, force: bool = True) -> None:
    ...
    **纯命令行**：重装运行期依赖，供 ``pipeline._reinstall_dependencies`` 与后续 UI 共用。
    ...
    2. 子进程内 ``core.infra.setup.core.ui_runtime.install_ui_runtime(force=...)``（BFF pip + FED ``npm install``，与 ``install.py`` 对齐）。
...
def spawn_mark_install_state_cli(repo_root: Path) -> None:
    """升级成功后写回 ``.ntq/install-state.json``（经 Setup 门面，子进程）。"""
    if _env_truthy("NTQ_UPDATE_SKIP_INSTALL_STATE"):
        return
    ...
    snippet = (
        ...
        "    REQUIREMENTS, UI_BFF_REQUIREMENTS, UI_FED_LOCKFILE,\n"
        "    sha256_file, fed_build_fingerprint, ui_dev_mode,\n"
        ")\n"
        "ts = time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime())\n"
        "Setup.runtime.mark('cli', success=True, fingerprints={"
        "'cli': {'requirementsHash': sha256_file(REQUIREMENTS), 'lastInstallAt': ts}})\n"
        "fp = {'python': {'uiRequirementsHash': sha256_file(UI_BFF_REQUIREMENTS)}}\n"
        "if ui_dev_mode():\n"
        "    fp['node'] = {'fedLockHash': sha256_file(UI_FED_LOCKFILE), 'lastInstallAt': ts}\n"
        "else:\n"
        "    fp['fedBuild'] = {'buildFingerprint': fed_build_fingerprint(), 'lastInstallAt': ts}\n"
        "Setup.runtime.mark('ui', success=True, fingerprints=fp)\n"
    )
    r = subprocess.run([str(py), "-c", snippet], cwd=str(repo_root), env=env)
    if r.returncode != 0:
        raise RuntimeError("NTQ updater: Setup.runtime.mark failed")
