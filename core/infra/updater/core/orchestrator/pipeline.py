...
**为何放在 ``userspace/system/updater``（发版源树见 ``core/infra/updater/core/orchestrator/``）：**

- **不能从 ``core/`` import 来跑流水线**：镜像时会覆盖 ``core/``。
- **不能放在 ``setup/``**：``setup/`` 在 ``managed_scope`` 内，覆盖时会换掉正在执行的脚本。
- **应放在 ``userspace/system/updater``**：不在发行 zip 的替换范围内；由 ``Updater.runtime.sync_orchestrator`` 写入。
...
def _trigger_core_extra_actions(ctx: UpgradeContext) -> None:
    """
    主流程结束后，子进程执行 ``core/infra/updater/core/post_upgrade`` 已注册的收尾动作。
...
def _refresh_install_state(ctx: UpgradeContext) -> None:
    """升级成功后写回 install-state，避免下次启动走全量安装。"""
    helper.spawn_mark_install_state_cli(ctx.repo_root)
...
def _reinstall_dependencies(ctx: UpgradeContext) -> None:
    ...
    **纯命令行** 重装运行期依赖（与后续 UI update 向导共用底层实现）。
    ...
    ``setup.core.ui_runtime.install_ui_runtime(force=True)``（BFF + FED）。
