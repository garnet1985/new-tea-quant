...
def resolve_updater_dir(start: Optional[Path] = None) -> Path:
    """定位 ``userspace/system/updater``；开发源树为 ``core/infra/updater/core/orchestrator``。"""
    ...
    if not candidates[0].joinpath("pipeline.py").is_file():
        ...
        for _ in range(8):
            for sub in (
                ...
                repo / "core" / "infra" / "updater" / "core" / "orchestrator",
            ):
    ...
    if not candidates[0].joinpath("pipeline.py").is_file():
        raise FileNotFoundError(
            "未找到 updater（userspace/system/updater 或 core/infra/updater/core/orchestrator）"
        )
    return candidates[0]
...
def repo_root_from_updater(updater_dir: Path) -> Path:
    ...
    if u.name == "orchestrator":
        return u.parents[4].resolve()
