...
- 包导出、方法可调用、返回类型、空白消息异常
- 对当前 userspace 的发现冒烟（目录缺失则跳过）
- 真调用仅在 `NTQ_ASSISTANT_LIVE=1` 时执行

**不负责**

- 扫描算法与 HTTP 解析细节（见 `core/__test__`）
...
| `test_list_and_get_callable` | `test_api.py` | 公开方法可调用；空名返回 `None`；含 set_api_key |
| `test_chat_rejects_blank_content` | `test_api.py` | 空白消息抛 `AssistantError` |
...
| `test_live_chat_pong` | `test_api.py` | 可选真调用 |
