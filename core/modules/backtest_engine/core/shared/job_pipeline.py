"""
Backtest Scheduler - General Job Pipeline

通用Job执行管道（线程/进程池 + on_result 回收），不包含任何DB特殊逻辑。

设计原则：
- General逻辑：ProcessPoolExecutor/ThreadPoolExecutor基本调度
- Queue/Batch模式
- 不包含DuckDB或其他DB的特殊逻辑
"""
...
class JobPipeline:
    ...
    def run(self, jobs: List[Job], *, run_name: str = "") -> DispatchResult:
        ...
        # General逻辑：直接调用，不包含DuckDB scope
...
from .executor import JobExecutor, create_job_executor
