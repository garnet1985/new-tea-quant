...
class MachineInfo:
    ...
    def resolve_max_workers(
        ...
        *,
        ...
        """Resolve ``max_workers`` including ``auto`` and profile caps."""
        from core.modules.backtest_engine.core.timeline_based.probe import WorkerProbe

        raw = performance.get("max_workers")
        if raw not in (None, "", "auto"):
            return max(1, int(raw))

        reserve_cores = MachineInfo.get_reserve_cores(performance)
        cap = MachineInfo._parse_max_parallel_jobs_cap(
