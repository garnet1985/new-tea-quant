...
位置：`core/modules/cli/dev/scripts/dependency_risk/`（经 `devcli` 调用；**无**独立 `devtools/` 树）。
...
# 或模块入口
python -m core.modules.cli.dev.scripts.dependency_risk
python -m core.modules.cli.dev.scripts.dependency_risk --ci-mode
```
