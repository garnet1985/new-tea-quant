...
使用方法:
    # 独立运行
    python -m core.modules.cli.dev.scripts.dependency_risk              # 标准检测
    python -m core.modules.cli.dev.scripts.dependency_risk --ci-mode    # CI 模式
    python -m core.modules.cli.dev.scripts.dependency_risk --fix        # 修复建议
...
class DependencyRiskDetector:
    ...
    def generate_report(self, risks: List[DependencyRisk], verbose: bool = False) -> str:
        ...
        report_lines.append(f"\n{i('sparkle')} 自动修复命令:")
        report_lines.append(f"   python -m core.modules.cli.dev.scripts.dependency_risk --fix")
