...
class StockAdjFactorEventsLoader(BaseDataContractLoader):
    ...
    def load(self, params: Mapping[str, Any]) -> Any:
        ...
        if not sid:
            raise ValueError("加载 stock.adj_factor.eventlog 失败：缺少 stock_id（请在 params 中提供 stock_id/id/entity_id）")
        sid = str(sid).strip()
