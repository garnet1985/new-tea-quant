"""
开发样本股票宇宙：由 ``data.json`` 的 ``use_sample_stock_list`` 统一配置。

名单文件位于 ``core/modules/data_manager/core/dev/sample_stock_list/``（进 Git）。

全局约束（下游无感）：
- 抓取侧：entity / dependencies 经 ``slice_stock_list`` 截断；
- ``PersistenceService`` 写 per-stock 表前经 ``filter_records`` 过滤。
- ``ListService`` 读路径信任 DB，不感知样本宇宙。
"""
...
class SampleUniverse:
    """
    系统内开发样本股票宇宙（分层抽样 CSV + data.json 开关）。

    经 ``DataManager.sample_universe`` 访问，不从包根导出。
    """

    DEFAULT_COUNT = 500

    _DIR = Path(__file__).resolve().parents[1] / "dev" / "sample_stock_list"
    _ids_cache: ClassVar[Optional[List[str]]] = None
    _logged: ClassVar[bool] = False

    @classmethod
    def count(cls) -> Optional[int]:
        return ProjectContext.config.get_use_sample_stock_list()

    @classmethod
    def ids(cls) -> List[str]:
        """当前配置的样本宇宙股票 id 列表；未配置时为空。"""
        return list(cls._load_ids())
    ...
    @classmethod
    def csv_path(cls, count: int) -> Path:
        n = int(count)
        if n <= 0:
            raise ValueError(f"use_sample_stock_list 须为正整数: {count!r}")
        return cls.directory() / f"stratified_{n}.csv"
    ...
    @classmethod
    def _load_ids(cls) -> List[str]:
        if cls._ids_cache is not None:
            return cls._ids_cache
    ...
    @classmethod
    def _filter_rows_by_pool(cls, rows: Sequence[Any], pool_ids: Sequence[str]) -> List[Any]:
        if not pool_ids:
            return list(rows)

        by_id: Dict[str, Any] = {}
        for row in rows:
            sid = cls._stock_id_from_row(row)
    ...
    def filter_records(
        ...
        *,
        ...
        """
        将 per-stock 写入行限制在样本宇宙内；未配置或非 per-stock 表时原样返回。
        """
        if not records:
            return records

        pool_ids = cls._load_ids()
        if not pool_ids:
            return records

        col = id_field or cls.stock_id_field_for_schema(schema)
        if not col:
            return records
    ...
    @classmethod
    def slice_stock_list(cls, rows: List[Any]) -> List[Any]:
        ...
        sliced = cls._filter_rows_by_pool(rows, pool_ids)
        if not cls._logged:
            cls._logged = True
            logger.info(
                ...
                len(rows),
                len(sliced),
                cls.csv_path(cls.count() or 0),
            )
        return sliced

    @classmethod
    def slice_stock_list_in_dependencies(cls, deps: Dict[str, Any]) -> Dict[str, Any]:
        if "stock_list" not in deps:
            return deps
        raw = deps.get("stock_list")
        if not isinstance(raw, list):
            return deps
        sliced = cls.slice_stock_list(raw)
        if sliced is raw:
            return deps
        out = dict(deps)
    ...
    def filter_paired(
        ...
        for idx, row in enumerate(main_records):
            sid = str(row.get("id") or "").strip()
            if sid not in allowed:
                continue
            main_out.append(row)
            raw_out.append(raw_records[idx] if idx < len(raw_records) else {})
        return main_out, raw_out

    @classmethod
    def prune(cls, data_manager: Any) -> int:
        """
        ...
        try:
            sql = f"DELETE FROM sys_tag_value WHERE entity_id NOT IN ({placeholders})"
            n = int(eng.execute_write(sql, params) or 0)
            if n:
                deleted += n
