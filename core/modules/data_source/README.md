...
**示例：**
- Schema 定义：`{"id": Field(...), "name": Field(...), "price": Field(...)}`
...
KLINE = DataSourceSchema(
    name="kline",
    schema={
        "id": Field(str, required=True),      # 股票代码，字符串，必需
        "date": Field(str, required=True),     # 交易日期，字符串，必需
        "open": Field(float, required=True),   # 开盘价，数字，必需
        "close": Field(float, required=True), # 收盘价，数字，必需
        "volume": Field(int, required=False), # 成交量，整数，可选
    }
)
```
