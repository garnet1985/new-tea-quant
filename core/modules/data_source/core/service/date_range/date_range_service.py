"""
DateRangeService - 基于 renew 配置计算 last_update_map 与实体日期范围。

委托 ``date_range_helper`` / ``RenewCommonHelper``；为 Handler 提供统一入口。
"""
...
class DateRangeService:
    ...
    def compute_last_update_map(self, context: Dict[str, Any]) -> Dict[str, Optional[str]]:
        """Phase 1：获取实体原始 last_update 映射（不考虑 renew_mode）。"""
    ...
    def compute_entity_date_ranges(
        ...
        """Phase 2：基于 last_update + renew_mode + renew_if_over_days 计算抓取窗。"""
