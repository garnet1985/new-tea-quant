...
def build_insights(report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    run_comparison = _run_comparison_insights(classical)
    if primary is None:
        ...
        next_steps = [
            ...
            f"确认 capture 已开启，并重新跑{_step_rerun_label(step)}后再看归因。",
        ]
        next_steps = _merge_next_steps(next_steps, run_comparison.get("next_steps") or [])
        ...
        return {
            ...
            "technical": _technical_shell(report, classical, skip_summary=skip_summary),
            "field_key": None,
            "other_fields": [],
            "run_comparison": run_comparison,
            "multivariate": _multivariate_insights(classical),
            "ml": _ml_insights(attribution),
        }
    ...
    findings = _key_findings(key, tiers, corr, skip_summary=skip_summary)
    headline = _headline(key, tiers, corr)
    chart_note = _chart_note(key, field, tiers, threshold, step=step)
    other_fields = _other_field_insights(fields, primary_key=key)
    multivariate = _multivariate_insights(classical)
    ml = _ml_insights(attribution)
    next_steps = _merge_next_steps(
        _next_steps(key, tiers, threshold, capture, fields),
...
def _multivariate_insights(classical: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if status in ("ok", "partial"):
        ranking = _rank_multivariate_features(block)
    ...
    if status in ("ok", "partial") and xgb.get("status") in ("ok", "partial"):
        ...
        for item in importance[:4]:
            ...
            ranking.append(
                {
                    "key": name,
                    "value": _fmt_num(score, 3) if score is not None else "-",
        ...
        changes.append(
            {
                ...
                "detail": (
                    f"参数从 {_fmt_plain(item.get('baseline'))} "
                    f"改成 {_fmt_plain(item.get('current'))}"
                ),
            }
        )
        ...
        changes.append(
            {
                "kind": "capture",
                "label": str(item.get("key") or "?"),
                "detail": _capture_diff_plain(item),
            }
        )
        ...
        changes.append(
            {
                ...
                "label": str(item.get("key") or "?"),
                "detail": (
                    f"样本量/覆盖从 {_fmt_plain(item.get('baseline'))} "
                    f"变为 {_fmt_plain(item.get('current'))}"
                ),
            }
        )
...
def _capture_diff_plain(item: Dict[str, Any]) -> str:
    ...
    if kind == "constant_value_changed":
        return (
            f"固定值从 {_fmt_plain(item.get('baseline'))} "
            f"变成 {_fmt_plain(item.get('current'))}"
        )
    ...
    if kind == "varying_range_changed":
        ...
        return (
            f"分布区间 "
            f"[{_fmt_num(baseline.get('min'))}~{_fmt_num(baseline.get('max'))}] → "
            f"[{_fmt_num(current.get('min'))}~{_fmt_num(current.get('max'))}]"
        )
