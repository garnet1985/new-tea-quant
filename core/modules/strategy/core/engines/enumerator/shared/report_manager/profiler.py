...
class DispatchPlanSnapshot:
    """调度计划快照（entity / slice 共用落盘；按 mode 填对应字段）。"""
        ...
        if getattr(plan, "reader_workers", None) is not None and getattr(
            ...
            return cls(
                ...
                probe=(
                    ...
                    else _probe_snapshot(getattr(plan, "probe", None))
                ),
            )
        return cls(
            mode="entity_based",
            ...
            probe=_probe_snapshot(getattr(plan, "probe", None)),
        )
...
class ProfilerPerformance:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        memory = _build_memory_usage(self.jobs, self.monitor, self.dispatch)
        ...
        probe_block = _build_probe_block(
            self.dispatch,
            self.jobs,
            self.monitor,
        ...
        payload: Dict[str, Any] = {
            ...
            "child_process": {
                ...
                "staged": _build_child_staged(phase_totals),
                "detail": {
                    "storage": storage_totals,
                    "contract": contract_totals,
                    "memory": memory,
                    "cold_start": _build_cold_start(self.jobs),
                    "failures": _build_failures(self.jobs, failed_jobs=self.failed_jobs),
                },
            },
...
def _memory_tier(
    *,
    ...
    out: Dict[str, Any] = {
        ...
        "actual_peak": peak,
        "estimate_accuracy": (
            _ratio_or_none(act, est) if est is not None else None
        ),
        "peak_overshoot": _peak_overshoot(peak, est) if est is not None else None,
    }
    if extra:
        out.update(extra)
    return out
...
def _build_time_distribution(
    *,
    ...
    if mode == "slice_based":
        ...
        return {
            "unit": "sec",
            "planning": _phase_bucket(planning_sec, wall),
            "read": _phase_bucket(read_sec, wall),
            "compute": _phase_bucket(compute_sec, wall),
            "report": _phase_bucket(finish, wall),
            "note": (
    ...
    return {
        "unit": "sec",
        "planning": _phase_bucket(planning_sec, wall),
        "load_data": _phase_bucket(load_sec, wall),
        "compute": _phase_bucket(compute_sec, wall),
        "report": _phase_bucket(finish, wall),
        "note": (
            "planning/report=流水线墙钟；"
...
def _build_slice_quick_summary(
    *,
    ...
    est_wait = _compute_wait_for_reader_sec(
    ...
    act_wait = _compute_wait_for_reader_sec(
    ...
    wait_acc = _ratio_or_none(act_wait, est_wait) if est_wait > 0 else (
    ...
    time_distribution = _build_time_distribution(
    ...
    return {
        ...
        "memory": {
            ...
            "avg_usage_rate": _ratio_or_none(actual_median, pool_mb),
            "peak_usage_rate": _ratio_or_none(actual_peak, pool_mb),
            "per_slice": {
                "reader": _memory_tier(
                    estimated=mb_r,
                    actual=actual_mb_r,
...
def _build_quick_summary(
    *,
    ...
    if str(mode or disp.mode or "") == "slice_based":
        return _build_slice_quick_summary(
    ...
    return {
        ...
        "memory": {
            "unit": "MB",
            "overall_available": round(pool_mb, 1),
            "avg_usage_rate": _ratio_or_none(concurrent_actual, pool_mb),
            "peak_usage_rate": _ratio_or_none(concurrent_peak, pool_mb),
            "per_entity": _memory_tier(
...
def _build_memory_usage(
    ...
    median_peak = _median(peaks) if peaks else 0.0
...
def _build_probe_block(
    ...
    *,
    ...
    if str(dispatch.mode or "") == "slice_based":
        return _build_slice_probe_block(dispatch, monitor, performance_config)
    ...
    memory = _build_memory_usage(jobs, monitor, dispatch)
    workers = max(1, int(dispatch.max_workers or 1))
    pool_mb = float(dispatch.memory_budget_mb or 0.0)
    ...
    if ran:
        ...
        estimate_ratio = _ratio(actual_concurrent_mb, predicted_concurrent_mb)
        safety_util = (
