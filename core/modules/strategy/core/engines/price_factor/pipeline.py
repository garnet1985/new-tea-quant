...
主流程::
    ...
    2. resolve_timeline     — 枚举 period start–end → 开市日列表（传给 BE timeline= 覆盖）
    ...
    4. BacktestEngine.entity_based — 调度自管；callbacks=JobExecutor；
       timeline= 覆盖（或 set_timeline）；未传则 BE 默认 CalendarService
...
边界:
    - 负责: 准备 jobs + timeline 覆盖 + JobExecutor 回调
...
class PriceFactorPipeline:
    ...
    def run_by_steps(cls, ctx: "SimulateRuntimeContext") -> Dict[str, Any]:
        ...
        timeline = cls.resolve_timeline(data)
        jobs = cls.build_jobs(data, timeline)
        run_result = cls.execute_backtest(jobs, timeline=timeline, data=data)
        raise NotImplementedError(
            "PriceFactorPipeline: execute 已接 BE（timeline= + JobExecutor）；"
            f"ReportManager 待实现 (entities={len(data.entity_ids)}, "
            f"version={data.version_id}, points={len(timeline.points)}, "
    ...
    @classmethod
    def resolve_timeline(cls, data: EnumVersionData) -> Timeline:
        """枚举 runtime period → 开市日 Timeline（作为 BE ``timeline=`` 覆盖）。"""
        return resolve_price_timeline(data)
    ...
    def build_jobs(
        ...
        return JobBuilder.build_jobs(data, timeline)
    ...
    def execute_backtest(
        ...
        *,
        ...
        """BE 自管调度；``timeline=`` 覆盖轴；钩子仅 ``JobExecutor.build_run_callbacks()``。"""
        if not jobs:
            return None
