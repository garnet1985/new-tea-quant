#!/usr/bin/env python3
"""Calendar slice enumerator worker (MVP: brute-force reload per slice)."""
...
class CalendarSliceEnumeratorWorker:
    """Single job: all stocks, calendar as-of loop, reload IO per slice."""
    ...
    def __init__(self, job_payload: Dict[str, Any]):
        ...
        self._stock_infos = self._load_stock_infos()
        self._load_user_strategy()
        self._states: Dict[str, _StockEnumState] = {}
        self._last_kline_by_stock: Dict[str, Dict[str, Any]] = {}

    def _load_stock_infos(self) -> Dict[str, Dict[str, Any]]:
        infos: Dict[str, Dict[str, Any]] = {}
        for sid in self.stock_ids:
            infos[sid] = self._load_single_stock_info(sid)
        return infos

    @staticmethod
    def _load_single_stock_info(stock_id: str) -> Dict[str, Any]:
        fallback = {
            "id": stock_id,
            "name": stock_id,
            "industry": "",
        ...
        try:
            ...
            if str(stock_id).upper() != "DUMMY":
                logger.warning("加载股票元数据失败: %s, error=%s", stock_id, exc)
        return fallback
    ...
    def run(self) -> Dict[str, Any]:
        ...
        try:
            open_dates = self._resolve_open_dates()
            slices = plan_calendar_slices(open_dates, self.slice_open_days)
            if not slices:
                return self._finish_bulk(success=True, stock_results=[])
            reporter = self._create_progress_reporter(open_dates, run_started_at=run_started_at)
            if reporter is not None:
                reporter.seed()
            carry: Dict[str, Any] = {}
            for slice_desc in slices:
                carry = self._run_slice(slice_desc, carry, reporter=reporter)
            stock_results = self._finalize_all_states()
            calendar_progress = reporter.finish() if reporter is not None else None
    ...
    def _resolve_open_dates(self) -> List[str]:
        if self.backtest_calendar is None:
            raise ValueError("calendar_slice 需要 backtest_calendar.open_dates")
    ...
    @staticmethod
    def _has_bar_on(state: _StockEnumState, as_of: str) -> bool:
    ...
    def _advance_holdings_for_day(self, state: _StockEnumState, as_of: str) -> None:
    ...
    def _force_exit_active_on_bar(self, state: _StockEnumState, as_of: str) -> None:
        ctx = self._bar_context(state, as_of)
        if ctx is None:
            return
        current_kline, prev_kline, _data = ctx
    ...
    def _bind_strategy_stock(self, state: _StockEnumState) -> None:
    ...
    def _finalize_all_states(self) -> List[Dict[str, Any]]:
