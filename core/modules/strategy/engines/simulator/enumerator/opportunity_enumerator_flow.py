...
class OpportunityEnumeratorFlow(BaseSimulationFlow):
    ...
    def preprocess(
        ...
        *,
        ...
        if reuse_plan["action"] == "REUSE_FULL":
            return {
                "strategy_name": strategy_name,
                "reuse_action": "REUSE_FULL",
                "reuse_version_dir": reuse_plan["version_dir"],
                "request_fingerprint": request_fingerprint,
            }
        # step3: create output version and build stock jobs
        if reuse_plan["action"] == "RUN_DIFF_STOCKS":
        ...
        global_extra_cache = self._impl.preload_global_cache(validated_settings, jobs)
        ...
        return {
            ...
            "reuse_base_version_dir": reuse_plan["version_dir"],
    ...
    def execute(self, preprocessed: Dict[str, Any]) -> Dict[str, Any]:
        if preprocessed.get("reuse_action") == "REUSE_FULL":
            return {"reused": True}
        # step1: run scheduled jobs with memory-aware batching
        job_results = self._impl.run_jobs(
            jobs=preprocessed["jobs"],
            global_extra_cache=preprocessed["global_extra_cache"],
            max_workers=preprocessed["max_workers"],
    ...
    def postprocess(
        ...
        aggregate = self._impl.aggregate_job_results(
            job_results=executed["job_results"],
            aggregate_profiler=preprocessed["aggregate_profiler"],
        )
        result_fingerprint = preprocessed["result_fingerprint"]
        if (
            preprocessed.get("reuse_action") == "RUN_DIFF_STOCKS"
            and preprocessed.get("cached_fingerprint") is not None
        ):
        ...
        self._impl.save_metadata(
            ...
            settings_snapshot=preprocessed["validated_settings"],
            enum_settings=preprocessed["enum_settings"],
        ...
        return self._impl.build_result_summary(
            ...
            reuse_action=preprocessed.get("reuse_action"),
            diff_stock_count=(
                len(preprocessed.get("missing_stock_ids") or [])
                if preprocessed.get("reuse_action") == "RUN_DIFF_STOCKS"
                else None
            ),
        )
