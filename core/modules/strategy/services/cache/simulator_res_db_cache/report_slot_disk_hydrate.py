...
def compact_enum_slot_for_cache(strategy_name: str, slot: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not vdir:
        return base
    report_path = PathManager.get_strategy_directory_simulation_enum(str(strategy_name).strip()) / vdir / _ENUM_REPORT_FILE
...
def resolve_enumerator_output_dir_from_downstream_report(
    ...
    *,
    ...
    if str(simulator_key or "").strip() == "capital_allocation":
        ...
        if sn and vd:
            meta = _read_json(
                PathManager.get_strategy_directory_simulation_capital(sn) / vd / "0_metadata.json"
            )
            if meta:
...
def build_enum_slot_from_enumerator_dir(
    ...
    if not (sn and vdir):
        return {}
    disk = _read_json(
        PathManager.get_strategy_directory_simulation_enum(sn) / vdir / _ENUM_REPORT_FILE
    )
    if not disk:
        return {}
    out = dict(disk)
...
def hydrate_enum_slot(strategy_name: str, slot: Dict[str, Any]) -> Dict[str, Any]:
    ...
    path = PathManager.get_strategy_directory_simulation_enum(sn) / vdir / rel
    ...
    if not isinstance(out.get("backtest_period"), dict):
        meta = _read_json(PathManager.get_strategy_directory_simulation_enum(sn) / vdir / "0_metadata.json")
...
def _resolve_capital_output_dir(
    ...
    *,
    ...
    if not (sn and needle):
        return None
    root = PathManager.get_strategy_directory_simulation_capital(sn)
    if not root.is_dir():
        return None
    best_name = ""
    best_id = -1
...
def compact_capital_slot_for_cache(
    ...
    *,
    ...
    if not vd:
        return out
    summary_path = (
        PathManager.get_strategy_directory_simulation_capital(str(strategy_name).strip()) / vd / _CAPITAL_SUMMARY_FILE
    )
...
def hydrate_capital_slot(strategy_name: str, slot: Dict[str, Any]) -> Dict[str, Any]:
    ...
    path = PathManager.get_strategy_directory_simulation_capital(sn) / vd / rel
    disk = _read_json(path)
    if not disk:
        return slot
    out = dict(disk)
