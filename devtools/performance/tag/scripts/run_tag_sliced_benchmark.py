...
# -*- coding: utf-8 -*-
"""
Calendar Sliced 模式 Tag 性能基准测试

目标：
  - 定义 calendar_sliced 模式的性能基线
  - 测试切片模式下的 IO/Compute 特性
  - 与 entity_timeline 模式对比

用法：
    # 运行默认配置测试
    python run_tag_sliced_benchmark.py

    # 使用更小的股票池（快速验证）
    python run_tag_sliced_benchmark.py --stock-limit 1000
...
RESULTS_BASE = _SCRIPT_DIR / "results" / "sliced"
TEST_BASE_TAGS_DIR = _SCRIPT_DIR.parent / "test_base_tags" / "calendar_sliced"
USERSPACE_TAGS_DIR = PROJECT_ROOT / "userspace" / "extensions" / "tags"
TEMP_SCENARIO_NAME = "bench_tag_sliced"
DEFAULT_STOCK_LIMIT = 5596  # 默认使用全量 A 股


@dataclass
class TagSlicedBenchmarkResult:
    """Calendar Sliced 模式 Tag 基准测试结果"""
    test_id: str
    timestamp: str

    # 配置
    scenario: str
    execution_mode: str = "calendar_sliced"
    stock_limit: int = 0
...
def _prepare_benchmark_scenario() -> str:
    ...
    # 复制 base tag 场景（已经是 calendar_slice 模式）
...
def _run_tag_scenario_sliced(
    ...
    *,
    ...
    """运行 tag 场景（sliced 模式）并返回是否成功和 report 路径"""
...
def run_sliced_benchmark(
    ...
    try:
        ...
        # 3. 运行 tag 场景
        success, report_path = _run_tag_scenario_sliced(
            scenario=scenario_name,
            stock_limit=stock_limit,
            enable_profile=True,
            dry_run=dry_run,
...
def generate_analysis_md(result: TagSlicedBenchmarkResult, output_path: Path) -> None:
    """生成 Markdown 分析报告"""
    ...
    if not has_data:
        md_content = f"""# Calendar Sliced Tag 性能基准报告
...
| **总任务数** | {result.total_jobs:,} | dispatch jobs (通常=1 for sliced) |
| **完成任务数** | {result.completed_jobs:,} | 成功的任务 |
| **失败任务数** | {result.failed_jobs:,} | 失败的任务 |
| **写入 Tag 数** | {result.saved_tag_values:,} | 写入的标签值数量 |
| **实体/秒** | {result.total_entities / result.wall_clock_seconds:.1f} | 实体处理速度 |
| **单实体耗时** | {result.wall_clock_seconds * 1000 / result.total_entities:.2f} ms | ms/实体 |

### Profile 细分（TagRunProfile）

| 阶段 | 耗时 (s) | 任务数 | 平均耗时 (ms) | 说明 |
|------|---------|--------|--------------|------|
| **Stage** | {result.stage_sec:.2f} | {result.stage_jobs} | {safe_div(result.stage_sec * 1000, result.stage_jobs):.1f} | 数据准备（全量切片） |
| **Execute** | {result.execute_sec:.2f} | {result.execute_jobs} | {safe_div(result.execute_sec * 1000, result.execute_jobs):.1f} | 标签计算（逐切片） |
...
- IO/Compute 比: **{safe_div(result.stage_sec, result.execute_sec):.2f}:1** {'(IO 密集)' if safe_div(result.stage_sec, result.execute_sec) > 1.5 else '(计算密集)' if safe_div(result.execute_sec, result.stage_sec) > 1.5 else '(平衡)'}
"""
        else:
            md_content += "\n*需要重新运行测试以获取此数据*\n"
    else:
...
## 📈 与 Entity Timeline 模式对比

| 维度 | Calendar Sliced | Entity Timeline | 差异分析 |
|------|-----------------|-----------------|----------|
| **Wall Time** | {result.wall_clock_seconds:.2f}s | ~参考值 | 待对比 |
| **并行度** | {result.parallelism_factor:.2f}x | ~参考值 | 待对比 |
| **任务数** | {result.total_jobs} | ~多任务 | 切片 vs 逐实体 |
| **适用场景** | 大规模批量标签 | 少量实时标签 | 不同目的 |
...
{"✅ 达标" if result.wall_clock_seconds < 120 else "⚠️ 需优化"} - Wall Time {'<' if result.wall_clock_seconds < 120 else '>'} 120秒 ({result.stock_limit:,} 实体，sliced 模式通常较慢但更省内存)

### 关键发现

1. **数据处理阶段 (Stage)** 占比 {safe_div(result.stage_sec, (result.stage_sec + result.execute_sec + result.report_sec)) * 100:.1f}%
   - {"IO 密集型：考虑本地数据库或缓存优化" if safe_div(result.stage_sec, (result.stage_sec + result.execute_sec + result.report_sec)) > 0.5 else "正常范围"}
...
Calendar Sliced 模式的特点：
- ✅ **内存友好**: 按时间切片处理，不一次性加载全量数据
- ✅ **适合大规模**: 适合数千实体的批量标签计算
- ✅ **IO 重叠**: 可预取下一片，隐藏 IO 延迟
- ⚠️ **启动开销**: 需要构建完整的时间切片计划

### 优化建议

{"**优先优化 Stage 阶段** (IO 瓶颈)" if safe_div(result.stage_sec, (result.stage_sec + result.execute_sec + result.report_sec)) > 0.5 else "**优先优化 Execute 阶段** (计算瓶颈)" if safe_div(result.execute_sec, (result.stage_sec + result.execute_sec + result.report_sec)) > 0.5 else "**整体均衡**，可针对性微调"}

---

*报告生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*
*由 run_tag_sliced_benchmark.py 自动生成*
"""
...
def main():
    parser = argparse.ArgumentParser(description="Calendar Sliced Tag Benchmark")
    parser.add_argument(
        "--stock-limit",
        type=int,
    ...
    print("=" * 60)
    print("  Calendar Sliced Tag 性能基准测试")
    print(f"  Project Root: {PROJECT_ROOT}")
    print(f"  Output Base: {RESULTS_BASE}")
    print(f"  Base Tag: calendar_sliced (from test_base_tags)")
    ...
    # 运行基准测试
    test_id = f"sliced_baseline_{args.stock_limit}"
    result = run_sliced_benchmark(
        test_id=test_id,
