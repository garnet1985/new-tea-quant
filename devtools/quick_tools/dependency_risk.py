...
使用方法:
    # 独立运行
    python -m devtools.quick_tools.dependency_risk              # 标准检测
    python -m devtools.quick_tools.dependency_risk --ci-mode    # CI 模式
    python -m devtools.quick_tools.dependency_risk --fix        # 修复建议
...
示例:
  ...
  %(prog)s --verbose        # 详细输出
    ...
    # 自动查找项目根目录（兼容 devtools/quick_tools 的调用方式）
    script_dir = Path(__file__).resolve().parent
    project_root = script_dir
    for _ in range(5):  # quick_tools/ → devtools/ → 项目根目录
