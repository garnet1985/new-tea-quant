...
- `a`（大版本）：当版本大于等于 `1` 时视为正式稳定版；大版本升级通常可能包含 **不兼容变更（breaking changes）**；
- `b`（小版本）：在不改变公共 API 的前提下的功能更新或重要修复；
- `c`（微小版本）：不改变用户使用方式的小更新或补丁（patch）。

当前代码库版本为：**`v0.1.0`**（首个对外开源的预发布版本）。  
`v0.0.x` 段代表在统一版本规范之前的历史内部里程碑版本。
...
完整更新记录请参见 `CHANGELOG.md`。这里仅保留最近几个重要版本摘要：
...
- 新增开源配套文档与基础 CI 流水线；
- 在 README 中补充项目定位、版本规范以及公共 API / 内部实现边界说明。
