...
- **极快的迭代速度**：基于枚举器的 枚举输出缓存，PriceFactorSimulator 无需再跑 on-bar 流程，只在已有机会上做轻量模拟，非常适合频繁调参
