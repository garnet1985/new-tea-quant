...
- **状态：** `beta`
- **引入版本：** `0.1.0`
- **描述：** `setup/core/steps/*/install.py` 直接执行时切到项目 venv
...
**描述：** 安装产物工厂（写入 `setup/init_userspace/`、`setup/init_data/`）
...
- **状态：** `beta`
- **引入版本：** `0.1.0`
- **描述：** 同步仓库 `userspace/` → `setup/init_userspace/` 并可选写 zip；`0` 成功
...
- **状态：** `beta`
- **引入版本：** `0.1.0`
- **描述：** 导出演示数据包到 `setup/init_data/`；`argv` 同 `python -m setup.core.scripts.init_data`
...
- **状态：** `beta`
- **引入版本：** `0.1.0`
- **描述：** 读取 `setup/core/steps/*/meta.json`，按依赖拓扑排序
