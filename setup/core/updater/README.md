...
| 路径 | 说明 |
|------|------|
| **`setup/core/updater/pipeline.py`** | 与运行时 **`userspace/updater/pipeline.py`** 同源；打 **init userspace zip** 时把该目录运行时文件放进包内 ``updater/``，解压后落到 **`userspace/updater/`**。 |
| **`setup/core/updater/run_apply.py`** | CLI 入口占位；本地可 ``python setup/core/updater/run_apply.py`` 调试。 |
...
| 2026-05-12 | 骨架固定在 ``setup/core/updater/`` → 随 init zip 进 ``userspace/updater/``（避免 ``core/``、``setup/`` 被替换时执行其中代码） |
