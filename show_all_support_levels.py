...
def show_all_support_levels():
    ...
    # 按价格排序显示
    sorted_lows = sorted(merged_lows, key=lambda x: x['price'])
    
    for i, low_point in enumerate(sorted_lows, 1):
        ...
        if 'price_range' in low_point:
            ...
            print(f"{i:2d}. {date} - 价格: {price:6.2f} 区间: [{range_min:5.2f}, {range_max:5.2f}] (±{range_percent:.1f}%, 宽度: {range_width:.2f}元)")
            print(f"    来源: {sources}")
        else:
    ...
    for i in range(len(sorted_lows) - 1):
        distance = abs(sorted_lows[i]['price'] - sorted_lows[i+1]['price'])
    ...
    if distances:
        ...
        print(f"支撑位平均距离: {avg_distance:.2f}元")
        print(f"最小距离: {min_distance:.2f}元")
        print(f"最大距离: {max_distance:.2f}元")
        
        # 评估投资实用性
        if avg_distance >= 2.0:
            print("✅ 投资实用性: 优秀 - 支撑位间距充足，便于设置止损止盈")
        elif avg_distance >= 1.0:
            print("✅ 投资实用性: 良好 - 支撑位间距适中，基本满足投资需求")
        elif avg_distance >= 0.5:
            print("⚠️  投资实用性: 一般 - 支撑位间距较小，需要谨慎操作")
        else:
            print("❌ 投资实用性: 较差 - 支撑位过于密集，不利于投资操作")
        ...
        for low_point in merged_lows:
            if 'price_range' in low_point:
                ...
                if (price_range['min'] <= level['max'] and price_range['max'] >= level['min']):
                    overlap_min = max(price_range['min'], level['min'])
                    overlap_max = min(price_range['max'], level['max'])
                    ...
                    print(f"    重叠区间: [{overlap_min:.2f}, {overlap_max:.2f}] (宽度: {overlap_width:.2f}元)")
                    found = True
                    break
